 ...
 *
 */
#include "AutomotiveCanV1_0Fuzzer_Integrated.h"
...
namespace android::hardware::automotive::can::V1_0::implementation::fuzzer {
    ...
    void CanBusSlcanFuzzer::fuzz() {
        preUp();
    }

    void CanBusVirtualFuzzer::fuzz() {
        preUp();
    }

    void CanBusNativeFuzzer::fuzz() {
        preUp();
    }

    void CanSocketFuzzer::fuzz(FuzzedDataProvider* mFuzzedDataProvider) {
       CanSocket::ReadCallback rdcb =
        [](const struct canfd_frame& frame, std::chrono::nanoseconds ns) {
            #pragma unused(frame,ns)
        };

        CanSocket::ErrorCallback errcb =
            [](int errnoVal) {
            #pragma unused(errnoVal)
        };

        std::string ifname = mFuzzedDataProvider->ConsumeRandomLengthString(mFuzzedDataProvider->remaining_bytes() / 2);
        auto can_socket = CanSocket::open(ifname, rdcb, errcb);
        if (can_socket && mFuzzedDataProvider->remaining_bytes() > 0) {
            struct canfd_frame frame;
            std::memset(&frame, 0, sizeof(frame));

            auto data = mFuzzedDataProvider->ConsumeBytes<uint8_t>(std::min(sizeof(frame.data), mFuzzedDataProvider->remaining_bytes()));
    ...
    void CanFuzzer::process(const uint8_t *data, size_t size) {
        ...
        invokeBus();
        // added direct calls
        findUsbDevice({mFuzzedDataProvider->ConsumeIntegralInRange<uint32_t>(0, kMaxSerialNumber)});
        std::string testString = mFuzzedDataProvider->ConsumeRandomLengthString(kMaxCharacters);
        readSerialNo(testString);
        getIfaceName(testString);
        isValidName(testString);
        // CanBusNativeFuzzer
        const std::string ifname = mFuzzedDataProvider->ConsumeRandomLengthString(kMaxCharacters);
        const uint32_t bitrate = mFuzzedDataProvider->ConsumeIntegral<uint32_t>();
        CanBusNativeFuzzer canFuzzer(ifname, bitrate);
        canFuzzer.fuzz();
        //CanBusSlcan
        CanBusSlcanFuzzer canFuzzer2(ifname, bitrate);
        canFuzzer2.fuzz();
        //CanBusVirtual
        CanBusVirtualFuzzer canFuzzer3(ifname);
        canFuzzer3.fuzz();
        //CanSocket
        CanSocketFuzzer canFuzzer4;
        canFuzzer4.fuzz(mFuzzedDataProvider);
    }
